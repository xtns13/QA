...
//        driver.findElements(by).size();
    ...
    public void waitAndLongPress(String locator, String error_message) {
        if (driver instanceof AppiumDriver) {
            ...
            WebElement element = waitForElementPresent(
                    ...
                    10);
            TouchAction action = new TouchAction((AppiumDriver)driver);
//        action.longPress((LongPressOptions) element).waitAction().release().perform();
        } else {
            System.out.println("Method waitAndLongPress() does nothing for platform " + Platform.getInstance().getPlatformVar());
    ...
    {
        String [] exploded_located = locator_with_type.split(Pattern.quote(":"), 2);
        String by_type = exploded_located[0];
        String locator = exploded_located[1];
